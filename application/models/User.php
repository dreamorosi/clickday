...
class User extends CI_Model
{
  ...
  function dispatchMail($email, $subject, $data, $template)
  {
    $this->load->library('mailchimp');
    $content = $this->load->view($template, $data, TRUE);
    return $this->mailchimp->sendMail($email, $subject, $content);
  }
	...
	function sendRecoveryMail($email, $code)
	{
		...
		return $this->dispatchMail($email, $subject, $data, $template);
	}
	...
	function setRecovery($email)
	{
		...
		if ($query->num_rows() > 0){
			...
			if ($this->db->affected_rows() > 0){
				$data['mc'] = $this->sendRecoveryMail($email, $code);
        $data['code'] = 200;
        return $data;
			}else{
				$data['code'] = 500;
